...
{
    internal static Dictionary<string, StarlightCommand> commands = new Dictionary<string, StarlightCommand>();
    ...
    public static bool RegisterCommand(StarlightCommand cmd)
    {
        if(cmd.ID.Contains(" "))
        {
            StarlightLogManager.SendMessage(translation("cmd.idcontainsspace", cmd.ID.ToLowerInvariant()));
            return false;
        }
        if (commands.ContainsKey(cmd.ID.ToLowerInvariant()))
        {
            StarlightLogManager.SendMessage(translation("cmd.alreadyregistered", cmd.ID.ToLowerInvariant()));
            return false;
        }
    ...
    public static bool UnRegisterCommand(string cmd)
    {
        ...
        StarlightLogManager.SendMessage(translation("cmd.notregistered", cmd.ToLowerInvariant()));
        return false;
    }
        ...
        foreach (string cc in cmds)
        {
            ...
            if (!string.IsNullOrWhiteSpace(c))
            {
                ...
                if (commands.ContainsKey(cmd))
                {
                    ...
                    if (MenuEUtil.isAnyMenuOpen)
                        StarlightLogManager.SendError(translation("cmd.unknowncommand"));
            }
        }
